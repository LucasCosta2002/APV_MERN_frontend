...
const Registrar = () => {
	...
	const handleSubmit = async e =>{
		...
		if(password.length < 6){
			setAlerta({ msg: "La contraseña debe contener minimo 6 caracteres", error: true})
			return;
		}
		...
		try {
			...
			setAlerta({
				msg: "Creado correctamente, revisa tu email y segui las instrucciones",
				error: false
			})
    ...
    <>
		...
		<div className='mt-20 md:mt-5 shadow-lg px-5 py-10 rounded bg-white'>
			...
			>
				...
				<div className="my-5">
					<label className="uppercase text-gray-600 block text-xl font-bold">Repeti tu Contraseña</label>
					<input 
						type="password" 
			...
			<nav className='mt-10 lg:flex lg:justify-between'>
				<Link 
					...
					to="/">¿Ya tenés una cuenta? Inicia Sesión
				</Link>
				<Link 
					className='block text-center my-5 text-gray-500'
